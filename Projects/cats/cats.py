...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    ps2 = [i for i in paragraphs if select(i)]
    if (k+1 > len(ps2)):
        return ''
    return ps2[k]
    # END PROBLEM 1
...
def about(subject):
    ...
    # END PROBLEM 2
    def about2(text):
        text_remove = remove_punctuation(text)
        text_split = split(text_remove)
        text_lower = [lower(s) for s in text_split]
        subject_lower = [lower(s) for s in subject]

        for i in subject_lower:
            for j in text_lower:
                if  i == j:
                    return True
        return False
...
def accuracy(typed, source):
    ...
    if type_len and source_len == 0:
        return 100.0
    elif type_len or source_len == 0:
        return 0.0
    count = 0
