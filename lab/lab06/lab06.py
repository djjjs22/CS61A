class Transaction:
    ...
    def changed(self):
        ...
        "*** YOUR CODE HERE ***"
        # if self.after > self.before or self.after < self.before:
        #     return True
        # else:
        #     return False
    ...
    def report(self):
        ...
        #         return "increased" + self.before + "->" + self.after
        #     if self.before > self.after:
        #         return "decreased" + self.before + "->" + self.after
        if self.changed():
            if self.before > self.after:
                verb = 'decreased'
            else:
...
class Server:
    ...
    def __init__(self):
        self.clients = {}
    # 方法内部，我们通过 email.recipient_name获取接收者的名称，并从 self.clients 字典中找到对应的客户端对象。
    # 然后，我们将 email 对象添加到该客户端的 inbox 列表中，表示邮件已送达接收者的收件箱。
    def send(self, email):
